#include "UserCode/ICHiggsTauTau/Analysis/HiggsNuNu/interface/EventShapeLorentz.h"
...
int EventShapeLorentz::calculate(){
  ...
  if(length<2){
    ...
    */
    ...
    if (nin>1){  
      if(Pt_sum_c<=0){
	cout<<"error in Pt_sum_c "<<Pt_sum_c<<endl;
      }
	...
	for(unsigned int i=0;i<nin;i++){
	  ...
	  if(dot_product_b>=0){
	    ...
	    if(rap==0){
	      eta_up+=Object_PtEtaPhiE_in[i].Pt()*Object_PtEtaPhiE_in[i].Eta();
	    }else{//rap==1 and rap==2
	      eta_up+=Object_PtEtaPhiE_in[i].Pt()*Object_PtEtaPhiE_in[i].Rapidity();
	    }	   
	    ...
	    if(rap==0){
	      eta_down+=Object_PtEtaPhiE_in[i].Pt()*Object_PtEtaPhiE_in[i].Eta();
	    }else{//rap==1 or rap ==2
	      eta_down+=Object_PtEtaPhiE_in[i].Pt()*Object_PtEtaPhiE_in[i].Rapidity();
	    }
	...
	if(nin!=(num_up+num_down)){
	  cout<<"error in counters: "<<nin<<"/"<<num_up<<"/"<<num_down<<endl;
	}
	...
	if (num_up>0){
	  ...
	  if(phi_up_aver>M_PI){
	    cout<<"error in phi_up_aver "<<phi_up_aver<<"/"<<num_up<<endl;
	  }
	}
	if(num_down>0){
	...
	//broadening or matrices are only 0 zero if at least two objects are in an hemisphere	
	for(unsigned int i=0;i<nin;i++){
	  dot_product_b =Object_PtEtaPhiE_in[i].Px()*ThrustAxis_c[0]+Object_PtEtaPhiE_in[i].Py()*ThrustAxis_c[1];
	  if(dot_product_b>=0){
	    if(num_up>1){
	      ...
	      if(rap==0){
		cenbroad_up+=Object_PtEtaPhiE_in[i].Pt()*sqrt(pow(Object_PtEtaPhiE_in[i].Eta()-eta_up,2)+pow(DeltaPhi(phi_up[index_up],phi_up_aver),2));
		matrixU[0][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*pow(Object_PtEtaPhiE_in[i].Eta()-eta_up,2);
		matrixU[0][1]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Eta()-eta_up)*(phi_up[index_up]-phi_up_aver);
		matrixU[1][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Eta()-eta_up)*(phi_up[index_up]-phi_up_aver);
	      }else{//rap==1 and rap==2
		cenbroad_up+=Object_PtEtaPhiE_in[i].Pt()*sqrt(pow(Object_PtEtaPhiE_in[i].Rapidity()-eta_up,2)+pow(DeltaPhi(phi_up[index_up],phi_up_aver),2));
		matrixU[0][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*pow(Object_PtEtaPhiE_in[i].Rapidity()-eta_up,2);
		matrixU[0][1]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Rapidity()-eta_up)*(phi_up[index_up]-phi_up_aver);
		matrixU[1][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Rapidity()-eta_up)*(phi_up[index_up]-phi_up_aver);
	      }
		...
		cout<<"problem in up phi: "<<fabs(phi_up[index_up]-phi_up_aver)<<"/"<<phi_up[index_up]<<"/"<<phi_up_aver<<endl;
	      }
	      index_up+=1;
	    }
	  }else if(num_down>1){
	    if(rap==0){
	      cenbroad_down+=Object_PtEtaPhiE_in[i].Pt()*sqrt(pow(Object_PtEtaPhiE_in[i].Eta()-eta_down,2)+pow(DeltaPhi(phi_down[index_down],phi_down_aver),2));
	      matrixD[0][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*pow(Object_PtEtaPhiE_in[i].Eta()-eta_down,2);
	      matrixD[0][1]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Eta()-eta_down)*(phi_down[index_down]-phi_down_aver);
	      matrixD[1][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Eta()-eta_down)*(phi_down[index_down]-phi_down_aver);
	    }else{//rap==1 or rap==2
	      cenbroad_down+=Object_PtEtaPhiE_in[i].Pt()*sqrt(pow(Object_PtEtaPhiE_in[i].Rapidity()-eta_down,2)+pow(DeltaPhi(phi_down[index_down],phi_down_aver),2));
	      matrixD[0][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*pow(Object_PtEtaPhiE_in[i].Rapidity()-eta_down,2);
	      matrixD[0][1]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Rapidity()-eta_down)*(phi_down[index_down]-phi_down_aver);
	      matrixD[1][0]+=Object_PtEtaPhiE_in[i].Pt()/Pt_sum_c*(Object_PtEtaPhiE_in[i].Rapidity()-eta_down)*(phi_down[index_down]-phi_down_aver);
	    }
	...
	cout<<"error matrixU offidag: "<<matrixU[0][1]<<"/"<<matrixU[1][0]<<" other elements "<<matrixU[0][0]<<"/"<<matrixU[1][1]<<endl;
      }
      if(matrixD[0][1]!=matrixD[1][0]){
	cout<<"error matrixD offidag: "<<matrixD[0][1]<<"/"<<matrixD[1][0]<<" other elements "<<matrixD[0][0]<<"/"<<matrixD[1][1]<<endl;
      }//test output for broken stuff
...
std::vector<float> EventShapeLorentz::Thrust_calculate (std::vector<TLorentzVector> Input_PtEtaPhiE){
  ...
  std::vector<float> Thrust_Axis_calc;
  std::vector<float> p_thrust_max_calc;
  std::vector<float> p_dec_1_calc;
  std::vector<float> p_dec_2_calc;
  std::vector<float> p_pt_beam_calc;
