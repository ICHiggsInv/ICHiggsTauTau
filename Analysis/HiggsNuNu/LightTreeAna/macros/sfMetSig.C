...
int sfMetSig() {//main
  ...
  if (region==2) filedir = "../output_allmindphi_1_metsig_3_4_mindphi_2/";
  ...
  std::string histname = "metnomu_significance";
  ...
  for (unsigned iD(0); iD<nDirs;++iD){
    ...
    if (iD==1) hist[iD] = (TH1F*)gDirectory->Get(histname.c_str())->Clone();
  ...
  else if (region==2) hist[1]->Scale(1/0.0779);
  ...
  ratioint->GetXaxis()->SetRangeUser(minX,maxX);
  //ratioint->GetYaxis()->SetRangeUser(0,0.26);
  ratioint->Draw();
  gStyle->SetOptFit(1111);

  TF1 *fit = new TF1("fit","exp([0]+[1]*x)",0,20);
  ...
  if (region != 2){
    ...
    //fit->Draw("same");
    res = fit->Integral(4.0,4.2)/0.2;//3.1416);
    reserr = fit->IntegralError(4.0,4.2)/0.2;//3.1416);
  } else {
    ratioint->Fit("pol1","+","same",minX,maxX);
    res = ratioint->GetFunction("pol1")->Integral(4.0,4.2)/0.2;
    reserr = ratioint->GetFunction("pol1")->IntegralError(4.0,4.2)/0.2;
  }
  std::cout << "Fit integral >4: " << res << std::endl;

  TLatex lat;
  lat.SetTextSize(0.05);
  char buf[500];

  sprintf(buf,"Expectation >4: SF=%3.3f #pm %3.3f",res,reserr);
  lat.DrawLatex(3.4,0.02,buf);
